...
typedef boost::mt19937 base_generator_type; //Docs say that this one is marginally slower but I'm unable to measure a repeatable difference. Supposedly this also has better random properties?
#endif
...
int main(int argc, char *argv[])
{
   ...
   /* Vector CrystalOrigin(0,-0.1,0);
    ...
    Vector BLOrigin( -3.0f, 0.0f, 5.0);
    VectorFromMap("Source", InputData, BLOrigin);
    
    double DirectionMinCosTheta = 0.0, DirectionMaxCosTheta = 0.0, DirectionMinPhi = 0.0, DirectionMaxPhi = 0.0;
     
    Vector SourceToOrigin = -1.0*BLOrigin;
	Vector BLNormal = SourceToOrigin.Normalized(); //BackLighterNormal
	
	
	Vector BLXAxis(1,0,0);
	Vector BLYAxis(0,1,0);
	
	double BLNormalTheta, BLNormalPhi;
	GetAlternateThetaPhi( BLNormal, BLNormalTheta, BLNormalPhi );
	
	//Rotate the Axes so that they're perpendicular to the normal.
	BLXAxis = RotateY(BLXAxis,BLNormalTheta);
	BLXAxis = RotateZ(BLXAxis,BLNormalPhi);
	
	BLYAxis = RotateY(BLYAxis,BLNormalTheta);
	BLYAxis = RotateZ(BLYAxis,BLNormalPhi);
	...
	double InputSourceRadius = 0.0300;
	double InputCollimatorRadius = 0.15;
	double InputCollimatorLength = 3.0;
		
	int NumSourceRadiusSteps = 0;
	int NumSourcePhiSteps = 40;
	
	IntFromMap( "NumSourceRadiusSteps", InputData, NumSourceRadiusSteps);
    IntFromMap( "NumSourcePhiSteps", InputData, NumSourcePhiSteps);
	
	bool bUsingPointSource = false;
	
	if( NumSourceRadiusSteps == 0) //point source
	{
		NumSourcePhiSteps = 1;
		MPI_cout << "Using Point source" << endl;
		bUsingPointSource = true;
	}
	
	
	double deltaSourceRadius = InputSourceRadius/double(NumSourceRadiusSteps);
	double deltaSourcePhi = (2.0*PI)/double(NumSourcePhiSteps);
	
	Vector CollimatorCentre = SourceToOrigin.Normalized() * InputCollimatorLength + BLOrigin;
	  
	double Divergence = atan( InputCollimatorRadius/InputCollimatorLength );
	MPI_cout << "Divergence: " << Rad2Deg(Divergence) << " Degrees" << endl;
	
	
    float MinTheta = 0.0;
    float MaxTheta  = Divergence;//Deg2Rad( SourceDivergence );
    float MinPhi = 0.0;
    float MaxPhi  = 2.0*PI;
    ...
    for( int CosThetaStep = 0; CosThetaStep <= NumThetaSteps; CosThetaStep++)
    {
        ...
        for( int PhiStep = 0; PhiStep <= NumPhiSteps; PhiStep++)
        {
            ...
            Vector CrystalIntersection(BLOrigin.x - (BLOrigin.z/SourceToCrystal.z)*SourceToCrystal.x,
                                       BLOrigin.y - (BLOrigin.z/SourceToCrystal.z)*SourceToCrystal.y,
	...
	//Vector SourceToCrystal = -1.0*BLOrigin;
    ...
    */
    ...
    AbsorbCoeffDataEnergy MuData( 0.4, MaxE, 10000, MuDataFilename.c_str());
    ...
    for( unsigned long int EnergyTick = WorkToDo.StartEnergyTick; EnergyTick < WorkToDo.EndEnergyTick; EnergyTick++)
    {
        ...
        float AbsorbCoeff = MuData.GetAbsorbCoeffDataPointEnergy( Energy );
		...
		for( int SourceRadiusStep = 0; SourceRadiusStep <= NumSourceRadiusSteps; SourceRadiusStep++)
		{
			double SourceRadius = SourceRadiusStep * deltaSourceRadius;
			
			double sigma = InputSourceRadius/3.0;
			
			
			if(bUsingPointSource == false)
			{
				cout << "Large source size currently not implemented properly! (" << __FILE__ << ":" << __LINE__ << ")" << endl;
				exit(1);
				double SourcePointRelativeIntensity = exp( -1.0*(SourceRadius*SourceRadius) / (2.0*sigma*sigma)  );
				//SpectrumCorrectedRepeats *= SourcePointRelativeIntensity;
			}
			
			
			
			
			for( int SourcePhiStep = 0; SourcePhiStep < NumSourcePhiSteps; SourcePhiStep++ ) //want to go around slightly less than 2 pi
			{
				double SourcePhi = SourcePhiStep * deltaSourcePhi;
		
				Vector Source = SourceRadius*cos(SourcePhi)*BLXAxis + SourceRadius*sin(SourcePhi)*BLYAxis + BLOrigin;
				
				if(bUsingPointSource)
				{
					Source = BLOrigin;
				}
				
				
				Vector SourceToCollimator = CollimatorCentre-Source;
				double SourceToCollimatorTheta = SourceToCollimator.GetTheta();
				double SourceToCollimatorPhi = SourceToCollimator.GetPhi();
				
				SourceToCollimator = SourceToCollimator.Normalized(); //vector from the point in the backlighter to the centre of the collimator pinhole
		
				for( int CollimatorCosThetaStep = 0; CollimatorCosThetaStep <= NumThetaSteps; CollimatorCosThetaStep ++)
				{
					double CollimatorCosTheta = MinCosTheta + double(CollimatorCosThetaStep)*deltaCosTheta;
					
					double SolidAngleCorrectedRepeats = SpectrumCorrectedRepeats;
					if(CollimatorCosThetaStep == 0 || CollimatorCosThetaStep == NumThetaSteps)
					{
					...
					for( int CollimatorPhiStep = 0; CollimatorPhiStep < NumPhiSteps; CollimatorPhiStep ++) //Q: Do I want to go around by 2pi or slightly less than 2pi? A: Slightly less
					{
						double CollimatorPhi = MinPhi + double(CollimatorPhiStep)*deltaPhi;
						
						Vector Direction( acos(CollimatorCosTheta), CollimatorPhi, true, true);
						//Direction = TransformToNewFrame(Direction, SourceToCrystalTheta, SourceToCrystalPhi);
						
						Direction = TransformToNewFrame(Direction, SourceToCollimatorTheta, SourceToCollimatorPhi);
						...
						for(int repeat = 0; repeat < CorrectedRepeats; repeat++)
						{
							...
							//0.5 is from only creating x-rays that point upwards.
								else if(R < ProbAbsorb)
								{
									...
									double CosTheta = uni();//only create x-rays with +ve z (theta 0-->pi/2)   0-->pi: uni()*2.0 - 1.0;
									if(CosTheta < DirectionMinCosTheta || CosTheta > DirectionMaxCosTheta)
									{
										continue;
									}									
									...
									//should check if it is quicker just to do this test. (it's not quicker)
									
									
									float Depth = fabs( AbsorptionLength * Direction.z ); //how deep xray gets into crystal
									
									
									float AbsorbCoeffFluo = MuData.GetAbsorbCoeffDataPointEnergy(FluoEnergy);
